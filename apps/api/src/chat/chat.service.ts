...
export class ChatService {
  ...
  private readonly faqs: FAQItem[] = [
    ...
    {
      keywords: ['tire price', 'tire cost', 'how much tire', 'tire installation', 'tire change price'],
    ...
    },
    {
      ...
      question: 'Wheel alignment price',
    },
    {
      ...
      question: 'Diagnostic service cost',
    },
    {
      ...
      question: 'Battery replacement price',
    },
    {
    ...
    },
    {
      ...
      question: 'Vehicle inspection price',
    },
    {
      ...
      question: 'Transmission service cost',
    },
    {
      keywords: ['ac price', 'air conditioning', 'ac recharge', 'freon', 'air filter price'],
      ...
      question: 'Air filter price',
    },
    {
      keywords: ['winter tire', 'winter tires', 'snow tire', 'winter tire package'],
      response: 'Winter tire packages including installation start at $400 for a set of 4 tires. Prices vary based on tire size and brand. Call for details at ' + this.supportPhone + '.',
      question: 'Winter tire package - call for details',
    },
    {
      ...
      question: 'General pricing information',
    },
    {
    ...
    },
    {
      ...
      question: 'Do you do oil changes?',
    },
    {
  ...
  async findSimilarQuestions(message: string): Promise<string[]> {
    const lowerMessage = message.toLowerCase().trim();
    ...
    const inputWords = lowerMessage.split(/\s+/).filter(word => word.length > 2);
    
    const matchedQuestions = new Set<string>();
    const scoredMatches: { question: string; score: number }[] = [];

    // Find FAQs that match any of the input keywords
    for (const faq of this.faqs) {
      if (!faq.question) continue;
      
      let score = 0;
      
      // Check if any FAQ keyword matches the input message
      for (const keyword of faq.keywords) {
        if (lowerMessage.includes(keyword)) {
          score += keyword.length; // Longer matches get higher scores
        }
        
        // Check if any input word matches any part of the keyword
        for (const word of inputWords) {
          if (keyword.includes(word)) {
            score += word.length * 0.5; // Partial matches get lower scores
          }
        }
      }
      
      if (score > 0) {
        scoredMatches.push({ question: faq.question, score });
      }
    }

    // Sort by score (highest first) and return top 5 unique questions
